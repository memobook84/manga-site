...
- スワイプナビ: volume.htmlで左右スワイプによる巻移動（右綴じ漫画と同じ向き: 左→右スワイプ＝次の巻、右→左スワイプ＝前の巻）
...
- クイックビュー: detail.htmlのシリーズ一覧で巻カバーをホバー（タッチ端末は常時表示）するとNetflix風の丸ボタンが出現、クリックで簡易ポップアップ（表紙・発売日・価格・レーベル・あらすじ・巻ページリンク・巻位置カウンター）。モバイルはポップアップを左右スワイプで前後の巻に切替（右スワイプ＝次、左スワイプ＝前。巻ページと同じ向き）
