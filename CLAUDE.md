...
- クイックビュー: detail.htmlのシリーズ一覧で巻カバーをホバー（タッチ端末は常時表示）するとNetflix風の丸ボタンが出現、クリックで簡易ポップアップ（表紙・発売日・価格・レーベル・あらすじ・巻ページリンク・巻位置カウンター）。モバイルはポップアップを左右スワイプで前後の巻に切替（指を右→左＝次、左→右＝前。巻ページと同じ向き）。アクセントカラーはテーマ紫 #4B2C82（ホバー #371F63）、表示中は背面スクロールをtouchmove preventDefaultで固定
