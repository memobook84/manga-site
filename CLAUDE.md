...
- ヘッダー配色: 背景は紫 #644476、サイトタイトルは白抜き（PC=#F7F7F7 / モバイル=#ffffff、どちらも !important）。モバイル（768px以下）は既定でタイトルを display:none にしてロゴ画像を出すが、検索トグル版のヘッダーでは文字を出す
- サイトのアクセント赤 #D8052E: 検索ページの見出し・枠線、ブログのボタン、ダークモードトグルON、単行本ページの「表示中の巻」枠線、作品ページのブックマークボタン（登録済み・ホバーは #B60426）
